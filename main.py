...
system_message = {
    ...
    Поле ID не нужно проверять на корректность, просто возьми цифру ID из исходных данных. Не нужно ругать за сильно подробные объяснения Элементов таблицы.
...
for row_dict in df.head(3).to_dict(orient='records'):
    json_str = json.dumps(row_dict)  # Исправлено: убрано [row_dict]
